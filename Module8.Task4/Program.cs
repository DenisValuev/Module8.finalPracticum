...
namespace Module8.Task4
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine(studentProp.Name + " " + studentProp.Group + " " + studentProp.DateOfBirth.ToString("d") + " " + studentProp.AverageScore);
            }
            string[] paths = { @"C:\Users", "Денис","Desktop"};
            ...
            try
            {
                ...
                {
                    directoryInfo.CreateSubdirectory("Students");
                    foreach (var studentProp in studentsToRead)
                    {
                        string[] path = { fullPath, "Students", "Группа " + studentProp.Group + ".txt" };
                        if (!File.Exists(Path.Combine(path)))
                        {
                            using (StreamWriter sw = File.CreateText(Path.Combine(path)))
                            {
                                sw.WriteLine(studentProp.Name + " " + studentProp.Group + " " + studentProp.DateOfBirth.ToString("d") + " " + studentProp.AverageScore);
                            }
                            countGroup++;
                        }
                        else
                    ...
                    Console.WriteLine($"Файлов создано: {countGroup}. Студенты отсортированы по группам ");
                }
                else
                {
                    Console.WriteLine("Путь задан некорректно!");
                }
            }
            catch (Exception e)
            {
